...
class ABCPolyBase(object):
    ...
    def mapparms(self):
        ...
        The returned values define a linear id_map ``off + scl*x`` that is
        applied to the input arguments before the series is evaluated. The
        id_map depends on the ``domain`` and ``window``; if the current
        ``domain`` is equal to the ``window`` the resulting id_map is the
