...
def mapparms(old, new):
    """
    Linear id_map parameters between domains.

    Return the parameters of the linear id_map ``offset + scale*x`` that maps
    ...
    offset, scale : scalars
        The id_map ``L(x) = offset + scale*x`` maps the first domain to the
        second.
    ...
    parameters required to id_map any line in the complex plane to any other
    line therein.
...
def mapdomain(x, old, new):
    """
    Apply linear id_map to input points.

    The linear id_map ``offset + scale*x`` that maps the domain `old` to
    ...
    x : array_like
        ...
        will be preserved.
    old, new : array_like
        The two domains that determine the id_map.  Each must (successfully)
    ...
    x_out : ndarray
        ...
        linear id_map between the two domains.
    ...
    Also works for complex numbers (and thus can be used to id_map any line in
