...
def buildusevars(m, r):
    ...
    revmap = {}
    if 'id_map' in r:
        for k in r['id_map'].keys():
            if r['id_map'][k] in revmap:
                outmess('\t\t\tVariable "%s<=%s" is already mapped by "%s". Skipping.\n' % (
                    r['id_map'][k], k, revmap[r['id_map'][k]]))
            else:
                revmap[r['id_map'][k]] = k
    if 'only' in r and r['only']:
        for v in r['id_map'].keys():
            if r['id_map'][v] in m['vars']:

                if revmap[r['id_map'][v]] == v:
                    varsmap[v] = r['id_map'][v]
                else:
                    outmess('\t\t\tIgnoring id_map "%s=>%s". See above.\n' %
                            (v, r['id_map'][v]))
            else:
                outmess(
