 ...
 *** it is a list of dictionaries containing `blocks':
     ...
     B['use'] --- dictionary of modules used in current block:
         {<modulename>:{['only':<0|1>],['id_map':{<local_name1>:<use_name1>,...}]}}
            ...
            if 'list' in mm and mm['list'] is not None:
                ...
                for l in ll:
                    if '=' in l:
                        ...
                        rl[l] = l
                    groupcache[groupcounter]['use'][name]['id_map'] = rl
            else:
                pass
        else:
            print(m.groupdict())
...
def use2fortran(use, tab=''):
    ...
    for m in list(use.keys()):
        ...
        if 'only' in use[m] and use[m]['only']:
            ret = '%s only:' % (ret)
        if 'id_map' in use[m] and use[m]['id_map']:
            c = ' '
            for k in list(use[m]['id_map'].keys()):
                if k == use[m]['id_map'][k]:
                    ret = '%s%s%s' % (ret, c, k)
                    c = ','
                else:
                    ret = '%s%s%s=>%s' % (ret, c, k, use[m]['id_map'][k])
                    c = ','
        if ret and ret[-1] == ',':
            ret = ret[:-1]
    return ret
